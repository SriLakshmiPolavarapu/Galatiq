...
def parse_decision(approval_text):
    """Pulls APPROVED/REJECTED plus reasoning out of the agent's free-text final answer."""
    ...
    if approved_pos == -1 and rejected_pos == -1:
        decision = "rejected"  # fail safe: if we can't tell, don't pay
...
def run_pipeline(invoice_path):
    ...
    ground_truth = validate_invoice(invoice)  # guaranteed-correct Python check, no LLM involved
